...
import { ShieldCheck, ArrowUpRight, Calendar, Sparkles, Eye } from "lucide-react";
...
export default function AchievementCard({ achievement }: AchievementCardProps) {
  ...
  return (
    <Link
      ...
      className="block h-full focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 rounded-3xl"
    >
      <motion.div
      ...
      >
        ...
        <div>
          ...
          <div style={{ transform: "translateZ(20px)" }} className="flex items-center justify-between mb-4">
            <span
              className={`px-3 py-1.5 rounded-full text-xs font-extrabold font-mono tracking-wider ${achievement.gradient} text-black shadow-md flex items-center gap-1.5`}
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>{achievement.badgeLabel}</span>
            </span>

            <div className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-500 dark:text-zinc-400 group-hover:text-indigo-600 dark:group-hover:text-cyan-400 group-hover:scale-110 group-hover:rotate-12 transition-all shadow-xs">
          ...
          <p
            ...
            className="text-slate-600 dark:text-zinc-300 text-sm leading-relaxed mb-5 line-clamp-3"
          >
        ...
        <div
          style={{ transform: "translateZ(15px)" }}
          className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-white/10 text-xs text-slate-500 dark:text-zinc-400"
        >
